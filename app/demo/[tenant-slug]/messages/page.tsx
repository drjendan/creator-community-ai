...
export default async function MessagesPage({ params }: { params: Promise<{ "tenant-slug": string }> }) {
  ...
  const { data: recipients } = await context.supabase.from("communication_message_recipients").select("id,message_id,read_at").eq("tenant_id", context.tenant.id).eq("user_id", context.user.id).is("archived_at", null);
