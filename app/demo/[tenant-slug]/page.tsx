...
export default async function TenantHomePage({ params }: { params: Promise<{ "tenant-slug": string }> }) {
  ...
  if (!tenant) notFound();
  const cards = [
    { title: episodes.length ? `${episodes.length} published episode${episodes.length === 1 ? "" : "s"}` : "No episodes published yet.", description: "Published episodes from this organization appear here.", action: "Browse Episodes", href: `/demo/${slug}/episodes`, icon: Headphones },
    { title: courses.length ? `${courses.length} published course${courses.length === 1 ? "" : "s"}` : "No courses published yet.", description: "Published learning experiences from this organization appear here.", action: "Browse Courses", href: `/demo/${slug}/courses`, icon: BookOpen },
    { title: events.length ? `${events.length} published event${events.length === 1 ? "" : "s"}` : "No upcoming events.", description: "Published events from this organization appear here.", action: "Browse Events", href: `/demo/${slug}/events`, icon: CalendarDays },
    { title: resources.length ? `${resources.length} published resource${resources.length === 1 ? "" : "s"}` : "No resources published yet.", description: "Published resources from this organization appear here.", action: "Browse Resources", href: `/demo/${slug}/resources`, icon: FileText }
  ];
  return (
    <main>
      <section className="upnexx-hero text-white">
        <Container className="py-20">
          ...
          <Button href={`/demo/${slug}/member`} className="mt-8">Open Member Dashboard <ArrowRight className="h-4 w-4" /></Button>
        </Container>
      </section>
