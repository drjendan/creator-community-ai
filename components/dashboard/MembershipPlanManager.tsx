...
type Plan = {
  ...
  sort_order: number; access_rules?: Record<string, boolean>;
};
...
export function MembershipPlanManager() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage("Audience membership plan saved.");
  ...
  return <div className="space-y-6">
    ...
    {loading ? <Card><p className="flex items-center gap-2 text-sm text-brand-600"><RefreshCw className="h-4 w-4 animate-spin" />Loading plans…</p></Card> : plans.length ? <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{plans.map((plan) => <Card key={plan.id} className="flex flex-col"><div className="flex items-center justify-between"><span className="rounded-full bg-accent-100 px-3 py-1 text-xs font-bold capitalize text-accent-800">{plan.plan_type}</span><span className="text-xs font-bold capitalize text-brand-500">{plan.status}</span></div><Users className="mt-6 h-8 w-8 text-accent-600" /><h2 className="mt-4 font-display text-2xl font-bold text-brand-900">{plan.name}</h2><p className="mt-2 text-sm leading-6 text-brand-600">{plan.description || "Audience membership"}</p><p className="mt-5 text-2xl font-extrabold text-brand-900">{plan.plan_type === "free" ? "Free" : `$${Number(plan.price_monthly).toFixed(2)}`}<span className="text-sm font-medium text-brand-500">{plan.plan_type === "paid" && "/month"}</span></p><ul className="mt-5 space-y-2 text-sm text-brand-600">{plan.community_access && <li className="flex gap-2"><Check className="h-4 w-4 text-success" />Community access</li>}{plan.ai_access && <li className="flex gap-2"><Check className="h-4 w-4 text-success" />{plan.ai_monthly_allowance} AI credits monthly</li>}<li className="flex gap-2"><Check className="h-4 w-4 text-success" />{plan.visibility === "public" ? "Publicly available" : "Invite only"}</li></ul><div className="mt-auto flex justify-end gap-2 pt-6"><button onClick={() => edit(plan)} aria-label={`Edit ${plan.name}`} className="rounded-lg p-2 text-accent-700 hover:bg-accent-50"><Edit3 className="h-4 w-4" /></button><button onClick={() => void remove(plan)} aria-label={`Delete ${plan.name}`} className="rounded-lg p-2 text-red-700 hover:bg-red-50"><Trash2 className="h-4 w-4" /></button></div></Card>)}</div> : <Card><p className="text-center text-brand-500">No audience membership plans yet. Add one or create a tenant with a membership template.</p></Card>}
  </div>;
}
