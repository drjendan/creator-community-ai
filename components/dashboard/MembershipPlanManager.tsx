...
type Plan = {
  ...
  billing_interval?: "monthly"|"annual"|"one_time"|"none"; enrollment_type?: "open"|"inquiry"|"invite_only";
  external_purchase_url?: string; website_shop_url?: string; external_booking_url?: string; contact_for_purchase?: string;
};
...
export function MembershipPlanManager() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/membership-plans", {
      ...
      body: JSON.stringify({
        ...
        billingInterval: data.get("billingInterval"), enrollmentType: data.get("enrollmentType"), externalPurchaseUrl: data.get("externalPurchaseUrl"), websiteShopUrl: data.get("websiteShopUrl"), externalBookingUrl: data.get("externalBookingUrl"), contactForPurchase: data.get("contactForPurchase"),
  ...
  return <div className="space-y-6">
    ...
    {open && <Card><div className="mb-5 flex items-center justify-between"><h2 className="font-display text-xl font-bold text-brand-900">{editing ? "Edit" : "Add"} audience plan</h2><button onClick={() => setOpen(false)} aria-label="Close"><X className="h-5 w-5" /></button></div><form onSubmit={save} className="grid gap-5 md:grid-cols-2">
      ...
      <Field label="Billing interval" htmlFor="membership-interval"><Select id="membership-interval" name="billingInterval" defaultValue={editing?.billing_interval??"monthly"}><option value="monthly">Monthly</option><option value="annual">Annual</option><option value="one_time">One time</option><option value="none">No payment / inquiry</option></Select></Field>
      <Field label="Enrollment" htmlFor="membership-enrollment"><Select id="membership-enrollment" name="enrollmentType" defaultValue={editing?.enrollment_type??"open"}><option value="open">Open enrollment</option><option value="inquiry">Inquiry only</option><option value="invite_only">Invite only</option></Select></Field>
      ...
      <Field label="External Purchase URL" htmlFor="external-purchase"><Input id="external-purchase" name="externalPurchaseUrl" type="url" defaultValue={editing?.external_purchase_url??""}/></Field>
      <Field label="Existing Website Shop URL" htmlFor="website-shop"><Input id="website-shop" name="websiteShopUrl" type="url" defaultValue={editing?.website_shop_url??""}/></Field>
      <Field label="External Booking URL" htmlFor="external-booking"><Input id="external-booking" name="externalBookingUrl" type="url" defaultValue={editing?.external_booking_url??""}/></Field>
      <Field label="Contact for Purchase" htmlFor="contact-purchase"><Input id="contact-purchase" name="contactForPurchase" defaultValue={editing?.contact_for_purchase??""} placeholder="Email or instructions"/></Field>
